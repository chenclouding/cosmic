...
<html>
  <head>
    ...
    <title>My JSP 'peerComponent.jsp' starting page</title>
  ...
  <body>
     <%
      ...
      Integer fatherId=0;
      String back ="peerComp";
     
      
      Integer layerId = new Integer(request.getParameter("layerId"));//获取从页面layer.jsp传来layerId    
	    ...
	    <%   for(Object p:peerCompList){ 
	     ...
	     %>
	         <tr>
		          ...
		          <td><a href="/fsmch/FSM/cosmicStandard/strategy/editPeerComponent.jsp?peerCompId=<%=peerCompId%>&layerId=<%=layerId%>&productId=<%=stratetyProductId%>"><IMG src="/fsmch/images/btnedit.gif" alt="修改" border="0" /></a></td>
		          <td><a href="/fsmch/FSM/cosmicStandard/strategy/delPeerComponent.jsp?peerCompId=<%=peerCompId%>&layerId=<%=layerId%>&productId=<%=stratetyProductId%>" onclick="return confirmdel();"><IMG src="/fsmch/images/btndel.gif" alt="删除" width="15" height="12" border="0" /></a></td>
		         <%if(compType==0 && measureStatus!=2){//组件类型是组合组件并且没有结束估算，则连接处是"添加对等组件" %>
		            <td><a href="javascript:window.name=='compFrame'?parent.location='/fsmch/FSM/cosmicStandard/strategy/addSubPeerComponent.jsp?fatherId=<%=peerCompId%>&layerId=<%=layerId%>&productId=<%=stratetyProductId %>&back=<%=back %>':window.location.href='/fsmch/FSM/cosmicStandard/strategy/addSubPeerComponent.jsp?fatherId=<%=peerCompId%>&layerId=<%=layerId%>&productId=<%=stratetyProductId%>&back=<%=back %>'">添加子对等组件</a></td>
