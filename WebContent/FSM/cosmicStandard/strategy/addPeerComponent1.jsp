...
<html>
  <head>
    ...
    <title>My JSP 'addPeerComponent1.jsp' starting page</title>
    ...
    <script language="javascript">
       ...
       // self.location("/fsmch/FSM/cosmicStandard/strategy/addPeerComponent.jsp?productId="+document.form0.productId.value);
        window.location.href("/fsmch/FSM/cosmicStandard/strategy/addPeerComponent.jsp?productId="+document.form0.productId.value);   
       }
  ...
  <body>
   ...
   <% 
     ...
     int productId = new Integer(request.getParameter("productId")); //获取从addPeerComponent.jsp页面传来的productId
     /*从数据库中把软件信息取出*/
     ...
     Iterator it2 = sp.getLayerByProductId(productId).iterator();
      String productName=fp.getProductName(); 
              ...
              <td>
	                请选择产品
			      	<select name="productId"  style="width:160px" onChange="productSelect();">
			      	   ...
			      	   <%
			      	    while(it1.hasNext()){
			      	        FsmProduct fp1 = (FsmProduct)it1.next();%>
			      	        <option value="<%=fp1.getId()%>"><%=fp1.getProductName()%></option>
			      	  <%}%> 			      		   
				    </select>
		        </td>
		         <td>
