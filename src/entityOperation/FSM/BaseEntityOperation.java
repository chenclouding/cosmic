...
import entityOperation.EntityOperationIF;
 ...
 * 
 */
public abstract class BaseEntityOperation{
	...
	public boolean insert(Object dataObject)throws HibernateException {
		...
		return false;          //替换学长的代码：throw e,因为若执行throw e，则不会再执行finally
	}
	/**
	 * 修改一条数据
	...
	//依据外键删除
	public boolean delByFKey(String tableName,String fkName,Integer fkValue)throws HibernateException {


		try {	

			List list = queryByFkey(tableName,fkName,fkValue);
	 ...
	 * @others  这里不能删除处于持久状态（persistent）的Object
	 */   
	public Object queryByColumnName(String tableName,String colName,int colValue) {    	
    	//初始化session and transaction
		Session session = null;
		Transaction tx = null;		
		try {		
	...
	public List queryByColumnNames(String tableName,String colName1,Integer colValue1,String colName2,Integer colValue2) {    	
		...
		try {		
			...
			//String sql = "from "+tableName+" as c where c."+colName1+"=4"+" and c."+colName2+"=0";
	 ...
	 * @throws HibernateException
	 *//*  
	public List queryByPwdAndUser(String tableName,String colValue1,String colValue2) {    	
    	//初始化session and transaction
		Session session = null;
		Transaction tx = null;		
		try {		
			session = openHibernateSession();
			tx = session.beginTransaction();	
			String sql = "from "+tableName+" as c where c.userName="+colValue1+" and c.password="+colValue2;
			//String sql = "from "+tableName+" as c where c."+colName1+"=4"+" and c."+colName2+"=0";
			Query query = session.createQuery(sql);
			query.setString("userName", colValue1);
			query.setString("password", colValue2);
